...
public class ExcelUtil
{
  ...
  public HSSFWorkbook doExportXLS(List dateList, String sheetname, boolean isEntity)
    throws IOException
  {
  ...
  public HSSFWorkbook createXLSEntityBulk(HSSFWorkbook wb, List<Object> dateList)
  {
    ...
    while (sublistIndex < dateList.size())
    {
      ...
      for (int i = 1; i <= subList.size(); i++)
      {
        ...
        for (int j = 0; j < this.beannames.length; j++) {
          BeanWrapper bw = new BeanWrapperImpl(bean);
          doSetCell(wb, sheet, (short)i, (short)j, bw
            .getPropertyValue(this.beannames[j]));
        }
  ...
  public void createUploadXLSEntity(HSSFWorkbook wb, HSSFSheet sheet, List<Object> dateList)
  {
    for (int i = 2; i <= dateList.size() + 1; i++) {
      ...
      for (int j = 2; j <= this.beannames.length + 1; j++) {
        ...
        BeanWrapper bw = new BeanWrapperImpl(bean);
        doSetCell(wb, sheet, (short)i, (short)j, bw
          .getPropertyValue(this.beannames[(j - 2)]));
      }
    }
  }
  ...
  public void setRowAutoHeight(ExcelExportUtil excelExport, Sheet sheet, int rowIndex, int columnIndex, String cellValue)
  {
  ...
  public void setHeaderWidth(ExcelExportUtil excelExport, Sheet sheet, int columnIndex, String headerValue, boolean flag)
  {
